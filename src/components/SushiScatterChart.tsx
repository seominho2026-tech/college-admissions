    ...
    <div className="relative bg-white border border-slate-200 p-5 rounded-xl shadow-sm">
      ...
      <div className="overflow-x-auto">
        <svg
          ...
          style={{ width: '100%', minWidth: '700px', height: '100%', maxHeight: '400px' }}
        >
          ...
          {data.map((record, index) => {
            ...
            const isHovered = hoveredPoint?.record === record;
            ...
            return (
              <circle
                ...
                cy={cy}
                r={isHovered ? 8 : 5}
                className={`cursor-pointer transition-all duration-150 ${fillColor}`}
        ...
        {/* Hover Tooltip Overlay (Absolute HTML inside React) */}
        {hoveredPoint && (
          <div
            className="pointer-events-none absolute z-20 w-64 rounded-xl border border-slate-200 bg-slate-950/95 p-3.5 text-white shadow-xl backdrop-blur-xs transition-opacity duration-150"
            style={{
              left: `${Math.min(width - 240, Math.max(10, hoveredPoint.x - 120))}px`,
              top: `${Math.max(10, hoveredPoint.y - 150)}px`,
            }}
          >
            <div className="mb-1 flex items-center justify-between">
              <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">
                {hoveredPoint.record.year || '24'}학년도 수시
              </span>
              <span
            ...
            <h5 className="font-bold text-sm tracking-tight text-white m-0">
              {hoveredPoint.record.university}
            </h5>
            <p className="text-[11px] text-slate-300 font-semibold mb-2">
              {hoveredPoint.record.department}
            </p>
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-1.5 border-t border-slate-800 pt-2 text-[10px] font-mono">
              <div>
                ...
                <span className="text-emerald-400 font-bold text-xs">
                  {hoveredPoint.record.grade.toFixed(2)} 등급
                </span>
              </div>
              <div>
                <span className="text-slate-400 block text-[9px]">환산 점수</span>
                <span className="text-slate-200 font-semibold">
                  {hoveredPoint.record.score !== null ? hoveredPoint.record.score.toFixed(1) : '-'} 점
                </span>
              </div>
              ...
              <div className="col-span-2 grid grid-cols-3 gap-1 border-t border-slate-900 pt-1.5 text-center">
                <div>
                  ...
                  <span className="text-slate-200 block text-[10px]">
                    {hoveredPoint.record.allSubjects !== null ? hoveredPoint.record.allSubjects.toFixed(2) : '-'}
                  </span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[8px]">국수영시</span>
                  <span className="text-slate-200 block text-[10px]">
                    {hoveredPoint.record.korMathEngSoc !== null ? hoveredPoint.record.korMathEngSoc.toFixed(2) : '-'}
                  </span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[8px]">국영수과</span>
                  <span className="text-slate-200 block text-[10px]">
                    {hoveredPoint.record.korEngMathSci !== null ? hoveredPoint.record.korEngMathSci.toFixed(2) : '-'}
                  </span>
                </div>
              </div>
            </div>
